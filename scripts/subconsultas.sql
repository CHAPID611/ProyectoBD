...
from (select * from userroles where idrol = 3) as ur
...
where idlesson in (select idlesson from lessons where idlanguage = 3);
...
select * from users u
where exists (select * from contributions where contributions.iduser = u.iduser);
